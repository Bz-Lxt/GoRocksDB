...
// ScanPrefix returns all key/value pairs whose user key begins with prefix,
// in ascending key order. The limit semantics match Scan (<=0 or >1000 are
// clamped). It is safe to call immediately after a Flush() with no pending
// writes — the freshly rotated (and possibly empty) active memtable is
// handled correctly.
func (db *DB) ScanPrefix(prefix []byte, limit int) ([]KV, error) {
	if db.closed.Load() {
	...
	if len(prefix) == 0 {
		...
		merge.Seek(encoding.SeekKey(prefix))
	}
	...
	for merge.Valid() && len(out) < limit {
		...
		// upper bound: stop once we leave the prefix range
		if len(prefix) > 0 && !bytes.HasPrefix(uk, prefix) {
			break
		}
